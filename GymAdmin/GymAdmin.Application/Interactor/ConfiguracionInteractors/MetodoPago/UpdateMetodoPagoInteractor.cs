...
public class UpdateMetodoPagoInteractor : IUpdateMetodoPagoInteractor
{
    private readonly IMetodoPagoService _metodoPagoService;
    private readonly IValidator<MetodoPagoDto> _validator;

    public UpdateMetodoPagoInteractor(IMetodoPagoService metodoPagoService, IValidator<MetodoPagoDto> validator)
    {
    ...
    public async Task<Result> ExecuteAsync(MetodoPagoDto metodoPagoDto, CancellationToken ct)
    {
        ...
        var result = await _metodoPagoService.UpdateAsync(metodoPago, ct);
        if (result.IsSuccess)
            return Result.Ok();
